...
def calculate_ml_correct(predicted_winner, actual_winner):
    ...
    Returns:
        int - 1 if correct, 0 if incorrect, None if actual_winner is missing
    """
...
def validate_with_actual_data():
    ...
    # ========================================================================
    # STEP 2: RESOLVE MATCH_IDS (prefer sportsradar_game_id, fallback to model_training_nba)
    # ========================================================================
    print("\n[STEP 2] Resolving match_ids...")

    # First, use sportsradar_game_id from the predictions table itself
    if 'sportsradar_game_id' in df_predictions.columns:
        ...
        print(f"  ✓ {has_id}/{len(df_predictions)} records have sportsradar_game_id in {TABLE_NAME}")
    else:
        df_predictions['match_id'] = np.nan
        print(f"  ⚠️  Column sportsradar_game_id not found in {TABLE_NAME}")

    # Fallback: for rows still missing match_id, look up from model_training_nba
    missing_mask = df_predictions['match_id'].isna()
    missing_count = missing_mask.sum()

    if missing_count > 0:
        print(f"  ⚠️  {missing_count} records missing sportsradar_game_id, falling back to {HISTORICAL_TABLE}...")
        try:
            query = f"""
            ...
            print(f"  ✓ Loaded {len(match_id_lookup)} records from {HISTORICAL_TABLE} for fallback")
        ...
        print(f"  ✓ All records have sportsradar_game_id, no fallback needed")
    ...
    if missing_match_ids > 0:
        print(f"  ⚠️  {missing_match_ids} records still missing match_ids (no source available)")
    ...
    for idx, row in df_valid.iterrows():
        ...
        if game_summary:
            try:
                ...
                # Do not store 0/null scores if the game is not yet completed
                # API can return 0 or null for games that haven't been played
    ...
    # Only push SETTLED records — do not update rows that have no actual data
    df_settled = df_validation[df_validation['status'] == 'SETTLED'].copy()

    if len(df_settled) == 0:
        print("\n  No SETTLED records to push — all games are still PENDING")
        print(f"  (Skipped {len(df_validation)} PENDING records, no DB update performed)")
        return
