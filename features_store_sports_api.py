...
# ============================================================================
# LOAD DATA FROM CSV — reads Future_sports_api.csv instead of Future.csv
# ============================================================================
def load_data():
    """Load data from CSV file"""
    try:
        possible_files = ['Future_sports_api.csv']
        df = None
        ...
        if df is None:
            raise FileNotFoundError("Could not find Future_sports_api.csv")
...
def prepare_data_for_db(df):
    ...
    if missing_id_count > 0:
        print(f"  Removed {missing_id_count} row(s) with missing game_identifier")
    ...
    if duplicates_removed > 0:
        print(f"  Removed {duplicates_removed} duplicate game_identifier(s) from CSV")
        ...
        for idx, row in df_clean.iterrows():
            try:
                ...
                if not game_id or pd.isna(game_id):
                    print(f"  Row {idx + 1}: Skipping - no game_identifier")
                    skipped_count += 1
                    continue
...
def main():
    print("="*80)
    print("STORING HISTORICAL FEATURES TO DATABASE (Sports API Pipeline)")
    print("="*80)
